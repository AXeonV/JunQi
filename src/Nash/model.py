...
class ActorCritic(nn.Module):
	...
	def act(self, state, available_actions=None, greedy=False):
		...
		if self.has_continuous_action_space:
			...
			if available_actions is not None:
				mask = torch.BoolTensor(available_actions).to(action_logits.device)
				action_logits = torch.where(mask, action_logits, torch.tensor(-1e8, device=action_logits.device))
	...
	def evaluate(self, state, available_actions=None):
		...
		if self.has_continuous_action_space:
			...
			if available_actions is not None:
				action_logits = torch.where(available_actions == 1, action_logits, torch.tensor(-1e8, device=action_logits.device))
			dist = Categorical(logits=action_logits)
			action_probs = dist.probs
...
class Nash1:
	...
	def select_action(self, state, id, available_actions=None, test=False):
		...
		with torch.no_grad():
			state = torch.FloatTensor(state).to(device)
			action, action_logprob, state_val = self.policy_old.act(state, available_actions, greedy=test)
		if not test:
			self.buffer[id].states.append(state)
			self.buffer[id].actions.append(action)
			self.buffer[id].logprobs.append(action_logprob)
			self.buffer[id].state_values.append(state_val)
		return action.item() if not self.has_continuous_action_space else action.detach().cpu().numpy().flatten()
	...
	def update(self, id):
		...
		for _ in range(self.K_epochs):
			_, logprobs, _, state_values, dist_entropy = self.policy.evaluate(old_states, old_actions)
